...
public class Diagnosis {
    ...
    public Diagnosis(int pat_id,int pat_rec_id,int doc_id) throws ClassNotFoundException, SQLException{
    ...
    public Diagnosis() throws ClassNotFoundException, SQLException{
        this.connection = Connectionz.getConnection();
    }
    
    private void write2DB() throws SQLException{
        String queryStr = "insert into Diagnosis (description, Doctor_id, Patient_id) values ('"+this.description+"', '"+this.doctor_id+"', '"+this.patient_id+"')"; 
    ...
    public boolean checkExistDesc() throws SQLException{
        ...
        String queryStr = "select Diagnosis_id from PatientRecord where id = ? and Patient_id = ?";
        ...
        if(this.resultSet.next()){
            this.id = this.resultSet.getInt("Diagnosis_id");
        }
        queryStr = "select description from Diagnosis where id = ?";
        this.pstatement = this.connection.prepareStatement(queryStr);
    ...
    private void updatePatientRecord() throws SQLException{
        String queryStr = "update PatientRecord set Diagnosis_id = ? where id = ?"; 
        this.pstatement = this.connection.prepareStatement(queryStr);
        this.pstatement.setInt(1, this.id);
